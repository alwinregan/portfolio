...
export default function CertificationsAdminPage() {
  ...
  const handleExport = async () => {
    const data = await exportModule('certifications');
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url;
    a.download = `certifications-${new Date().toISOString().slice(0, 10)}.json`; a.click();
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const arr = Array.isArray(json) ? json : (json.certifications ?? []);
      const result = await importModule('certifications', arr);
